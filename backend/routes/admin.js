  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (!row.StudentID || !row.StudentName || !row.CompanyName || !row.Package) {
          results.errors.push(`Row ${i + 2}: Missing required fields`)
          continue
        }

        // Validate student exists
        const student = await User.findOne({ userId: row.StudentID, role: "student" })
        if (!student) {
          results.errors.push(`Row ${i + 2}: Invalid student ID ${row.StudentID}`)
          continue
        }
        ...
        const existingPlacement = await Placement.findOne({
          ...
          yearOfPlacement: row.YearOfPlacement || new Date().getFullYear(),
        })

        if (existingPlacement) {
          results.errors.push(`Row ${i + 2}: Placement already exists for student ${row.StudentID}`)
          continue
        }
        ...
        const placement = new Placement({
          ...
          jobRole: row.JobRole || "",
          placementType: row.PlacementType || "campus",
          addedBy: req.user._id,
        })
    ...
    await logActivity(
      "placements_uploaded",
      `${results.success} placement records uploaded via Excel`,
      req.user._id,
      "Placement",
      null,
      { successCount: results.success, errorCount: results.errors.length },
    )

    res.json({
      message: `Upload completed. ${results.success} records added successfully.`,
      results,
    })
  } catch (error) {
