...
const MaterialsTab = ({ course, showForm, setShowForm, onUpdate }) => {
  const { user } = useAuth()
            ...
            <div className="flex items-start justify-between">
              ...
              <div>
                {material.type === "link" ? (
                  ...
                  <a
                    href={`http://localhost:5000/${material.file?.path}`}
                    target="_blank"
...
const AssignmentsTab = ({ course, showForm, setShowForm, onUpdate }) => {
  const { user } = useAuth()
...
const AssignmentCard = ({ assignment, courseId, onUpdate }) => {
  const { user } = useAuth()
